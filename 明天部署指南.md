# 🚀 明天部署指南 - IMAP 邮箱服务 + 封面图优化

> **部署日期**: 2025-11-14
> **修改内容**:
> 1. 支持 IMAP 邮箱服务(QQ/163/Gmail),不再依赖代理
> 2. 封面图从 banner 图改为第一条新闻的图片

---

## 📋 三步部署流程

### 第一步: 本地提交代码到 Git

```powershell
cd E:\xProject\Plab-Rundown

# 添加所有修改
git add .

# 提交代码
git commit -m "feat: 支持IMAP邮箱服务 + 优化封面图选择"

# 推送到远程
git push
```

---

### 第二步: 服务器拉取代码

**宝塔面板终端**或 **SSH 登录**:

```bash
# 登录服务器
ssh root@159.75.238.79

# 进入项目目录
cd /root/Plab-Rundown

# 拉取最新代码
git stash
git pull
git stash pop
```

---

### 第三步: 上传 .env 文件

**使用宝塔面板**:

1. 打开宝塔面板文件管理
2. 进入 `/root/Plab-Rundown/` 目录
3. 上传本地的 `.env` 文件(覆盖服务器上的)

**本地 `.env` 文件位置**: `E:\xProject\Plab-Rundown\.env`

**确保 `.env` 包含以下内容**:
```bash
# IMAP 邮箱配置
EMAIL_USERNAME=jasonfnn@qq.com
EMAIL_PASSWORD=oznbwwzuewcgbfac

# 其他配置...
```

---

### 第四步: 重启容器

**宝塔面板终端**:

```bash
cd /root/Plab-Rundown/deploy

# 停止容器
docker-compose down

# 重新构建
docker-compose build

# 启动容器
docker-compose up -d
```

---

### 第五步: 测试验证

**宝塔面板终端**:

```bash
# 测试 IMAP 连接
docker exec plab-rundown python -c "from src.email.factory import create_email_client; client = create_email_client(); print('✅ 邮箱连接成功!')"

# 手动执行一次完整流程
docker exec plab-rundown python workflow.py

# 查看日志
docker logs plab-rundown --tail 100
```

**预期结果**:
- ✅ 邮箱连接成功
- ✅ 获取邮件成功
- ✅ 找到封面图片(第一条新闻)
- ✅ 草稿创建成功

---

## ✅ 部署完成!

明天早上 **06:00** 会自动执行任务,到时候检查微信草稿箱即可。

---

## 🔧 如果出问题

### IMAP 连接失败
```bash
# 检查环境变量
docker exec plab-rundown printenv | grep EMAIL

# 确认授权码: oznbwwzuewcgbfac
```

### 找不到邮件
```bash
# 手动检查 QQ 邮箱
# 登录 jasonfnn@qq.com,查看是否有来自 news@daily.therundown.ai 的邮件
```

### 回滚到 Gmail API
```bash
# 修改 config/config.yaml
# 将 provider: "imap" 改为 provider: "gmail_api"

# 重启容器
cd /root/Plab-Rundown/deploy
docker-compose restart
```

---

**祝部署顺利! 🚀**

